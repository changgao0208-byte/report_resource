...
\usepackage{acl}      % sets 11pt font, 2.5cm margins automatically
\usepackage{times}
\usepackage{latexsym}
...
A Comparative Corpus Analysis of Scientific Abstracts}

\author{Rui Liu \quad Dekai Fan \quad Karen Coutinho \quad Guruanand Reddy Batapati \quad Chang Gao}
...
Scientific abstracts show how researchers frame problems and key findings. This study analyzes how the framing of NLP research has changed based on a corpus of arXiv abstracts (1991-2021), comparing representation methods (TF-IDF, SBERT and LDA) and evaluating temporal shifts by keyword and semantic similarity measures. Centroid trajectories are visualised through PCA and UMAP projections. All four Axis 1 methods reflect a single dominant breakpoint at the 2011-2015 to 2016-2021 transition, which is consistent with the rise of transformer-based pre-training. The Axis 2 keyword shift confirms the same transition and cosine distance results uncover a layered pattern beneath this consensus: lexical change peaks first, semantic change later, and topical change last. NLP's paradigm shift propagated upward through representational layers instead of occurring simultaneously. 

\noindent\textbf{Githublink:}\url{https://github.com/guruanand20/nlp-corpus-analysis.git}
\end{abstract}
...
The source corpus was the publicly available \texttt{gfissore/arxiv-abstracts-2021} dataset, which contains arXiv metadata and abstracts up to the end of 2021. From this larger dataset, the arXiv subject labels \texttt{cs.CL} and \texttt{cmp-lg} were used to filter papers related to NLP. In order to get cleaner data, publication years were extracted from arXiv identifiers, and records without reliable year information were removed. The final NLP corpus contains 32,221 abstracts covering 1994-2021.

For historical comparison, the corpus was divided into six period bins: 1991-1995, 1996-2000, 2001-2005, 2006-2010, 2011-2015, and 2016-2021. Table~\ref{tab:corpus-summary} shows the number of abstracts and actual year range in each period. Although the first bin is labeled 1991-1995, the earliest records after cleaning begin in 1994. As shown in Figure~\ref{fig:period-counts}, ,the 2016-2021 period accounts for approximately 88.16\% of all abstracts, which shows the corpus is strongly imbalanced. Therefore, results involving this period should be interpreted carefully because it contains far more papers than earlier periods.

\begin{table}
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrl p{3cm}}
\hline
Period & Count & Years & Era \\
\hline
1991--1995 & 448 & 1994--1995 & Statistical \\
1996--2000 & 659 & 1996--2000 & Statistical \\
2001--2005 & 279 & 2001--2005 & Machine Learning \\
2006--2010 & 364 & 2006--2010 & Machine Learning \\
2011--2015 & 2066 & 2011--2015 & Deep Learning \\
2016--2021 & 28405 & 2016--2021 & Transformer \\
\hline
\end{tabular}
\caption{Statistics of the filtered NLP corpus.}
\label{tab:corpus-summary}
\end{table}

\begin{figure}
\centering
\includegraphics[width=0.85\linewidth]{figures/period_counts.png}
...
% ── Member 3: Requirements for a good solution ───────────────────────────
A reliable method for tracking change in NLP research must meet four requirements. First, it must be interpretable: the output should point directly to specific words or phrases, not just numerical scores that require further explanation. Second, it must account for corpus imbalance: because the 2016--2021 period contains far more abstracts than earlier periods, any comparison must normalise for period size to avoid mistaking quantity for significance. Third, it must capture both directions of change: identifying not only which terms became prominent but also which terms disappeared from the research vocabulary. Fourth, it must be transparent enough for readers to verify the findings without specialist tools. These four requirements motivate the two analytic axes in this study. Axis 1 examines how the choice of text representation of TF-IDF versus semantic embeddings shapes what patterns become visible. Axis 2 examines how the choice of comparison method, keyword shift analysis versus cosine distance determines what kind of change can be detected and explained.
...
The dataset was filtered from 1,999,486 abstracts to retain papers categorised under \texttt{cs.CL} and \texttt{cmp-lg}, resulting in 32,221 abstracts from 1994 to 2021, with the inclusion of \texttt{cmp-lg}, the predecessor of \texttt{cs.CL}. It ensures coverage of early NLP research and avoids discontinuities in the dataset. Publication years were extracted from both legacy and modern arXiv identifier formats, and documents were grouped into six five-year periods (1991-1995 to 2016-2021) to ensure comparability across time, with 2021 merged into the final bin due to limited sample size. Each period was further associated with a corresponding NLP technical paradigm (Era) for interpretation.
...
Based on the TF-IDF baseline, SBERT and LDA captures patterns that lexical overlap may miss. TF-IDF represents abstracts through word and phrase frequency, while SBERT focuses on sentence meaning and LDA focuses on topic structure. These two methods provided another view of the same historical change.

SBERT was implemented with the \texttt{all-MiniLM-L6-v2} Sentence-Transformers model, which maps each abstract into a dense vector with 384 dimensions \citep{allminilm_modelcard}. The model produced an embedding matrix of shape \texttt{(32221, 384)} with the full cleaned corpus. Additionally, Sentence-BERT places texts with similar meanings close together in vector space \citep{reimers2019sentence}. Compared with TF-IDF, SBERT can capture related ideas even when they are written with different words. The hypothesis is that SBERT will reveal semantic drift that is partly hidden from TF-IDF.

LDA was used to model topic structure at the corpus level \citep{blei2003latent}. After token filtering, 32,217 abstracts remained, with a dictionary of 12,860 terms. Each abstract was represented as a topic proportion vector, where each dimension gives the probability weight of one latent topic. The different $K$ values, including $K=8$, $K=10$, and $K=15$, were compared using coherence, with scores of 0.5101, 0.5082, and 0.5127. Since $K=15$ achieved the highest coherence and still produced interpretable topics, it was selected as the final setting. The hypothesis is that LDA will show changes in research themes more clearly than TF-IDF, because it groups related words into shared topics rather than treating each word separately.
...
\subsection{Keyword shift Representations}
...
The analysis was applied to all four representations - TF-IDF unigram, TF-IDF bigram, SBERT and LDA - producing four independent distance timelines. Comparing these timelines tests whether different representation choices lead to different conclusions about the timing of NLP's major paradigm shifts. We hypothesize that TF-IDF based distances will detect the earliest breakpoint since vocabulary change is a surface level signal that precedes deeper conceptual shifts. SBERT distances are expected to detect a later breakpoint, reflecting when the semantic meaning of research fundamentally changed rather than just its terminology. LDA distances are expected to show the most gradual change overall, since broad research topics shift more slowly than individual words or semantic representations. 
...
\subsection{PCA and UMAP Visualization}
...
TF-IDF scores are used as the primary metric to track vocabulary trajectories across time, where term importance reflects relative frequency weighted by inverse document frequency. To ensure comparability across periods with highly imbalanced document counts, term importance is aggregated using the mean rather than the sum. However, this metric treats each term independently, limiting its ability to capture semantic equivalence across different surface forms.
...
These findings confirm both hypotheses: TF-IDF Unigram captures the broad paradigm shift, while Bigram reveals compound terms such as language model and attention mechanism that are not visible at the unigram level. This pattern is consistent with the keyword shift analysis (Section~\ref{sec:keyword-results}), which later identifies similar neural vocabulary emerging from 2011-2015 onwards. And the results indicate a major transition between 2011–2015 and 2016–2021, with 2006–2010 acting as a transitional phase. The observed trends also align with established developments in NLP, including the rise of neural approaches and transformer-based models \citep{vaswani_attention_2023, devlin_bert_2019}.

TF-IDF also exhibits limitations that can affect interpretation. For instance, semantically equivalent terms such as \textit{language model} and \textit{language modeling} are treated as separate features, which may fragment the representation of key concepts and underestimate their true importance. Further, earlier periods with smaller sample sizes are more sensitive to outliers, which can distort term importance rankings.
...
SBERT and LDA were evaluated as complements to the TF-IDF baseline. For SBERT, cosine distance between period centroids was used to measure semantic drift. As shown in Figure~\ref{fig:sbert_distance}, nearby periods have smaller distances, while distant periods are more separated. The distance between 1991-1995 and 1996-2000 is only 0.015, but it rises to 0.259 between 1991-1995 and 2016-2021. This supports the hypothesis that SBERT captures gradual semantic drift rather than one sudden break. The relatively low distance between 2011-2015 and 2016-2021 also suggests that the final period continued some semantic patterns that were already present before 2016.

\begin{figure}
\centering
\includegraphics[width=0.85\linewidth]{figures/sbert_distance.png}
\caption{SBERT semantic distance across time periods.}
\label{fig:sbert_distance}
\end{figure}


In terms of LDA, mean topic proportions were used to follow changes in thematic structure. The results show broader theme changes than the TF-IDF word trends, because related vocabulary is grouped into shared topics. As shown in Figure~\ref{fig:lda_topic_trends}, Topic 8, related to parsing, grammar, and syntax, declines over time. In contrast, Topic 6, related to neural architectures, attention, and transformers, rises strongly in later periods. Topic 11, related to sentiment, social media, and bias detection, also becomes more visible after 2006-2010, while Topic 1, machine translation and multilingual NLP, remains relatively stable.

\begin{figure}
\centering
\includegraphics[width=0.85\linewidth]{figures/lda_topic_trends.png}
\caption{Major LDA topic trends across time periods.}
\label{fig:lda_topic_trends}
\end{figure}

Overall, these findings are consistent with the TF-IDF result that NLP moved away from grammar and syntax vocabulary and towards neural models and transformer research. SBERT adds evidence of gradual semantic drift, while LDA shows that vocabulary change reflects broader changes in research themes. The main limitation is interpretability: SBERT shows how much periods differ, but does not indicate which terms caused the difference, and LDA topic labels require manual judgement.



% ── Member 5 supplement ──

\subsection{PCA and UMAP Trajectories}
\label{sec:results-traj}

Figure~\ref{fig:pca-all} shows the centroid trajectories of the PCA for the four representations. The most significant characteristic is their convergence: regardless of whether the study is represented as vocabulary frequency, sentence embedding, or subject distribution, the maximum centroid shift occurs at the same transition point (Table~\ref{tab:traj-stats}). The 2011-2015 to 2016-2021 step dominates total trajectory length in every panel, which is consistent with the emergence of pre-trained language models based on transformers after 2017\citep{pramanick_diachronic_2023}. This confirms the PCA hypothesis that a clear and consistent overall trajectory can be observed in all four representations.
...
The displacement magnitudes differ meaningfully. SBERT shows the largest total trajectory (0.454) and single step (0.197), consistent with the results of Section~\ref{sec:results-sbert} that semantic embeddings capture a broader and more gradual drift than lexical methods. TF-IDF Bigram's much shorter total path (0.034 versus 0.133 for Unigrams), suggests that composite NLP terms are more stable than individual terms. PCA outputs are interpretable across four panels because they share a common Euclidean scale. Compared to UMAP, this cross-method interpretability is a significant advantage, since the magnitude of the distance across the panels in UMAP is not linearly meaningful.
...
\subsection{Keyword Shift Analysis}

Keyword shift analysis identifies vocabulary change at a more granular level than TF-IDF by naming the specific terms that entered and left the top-ranked lists across periods, rather than describing the general direction of change. Mean TF-IDF scores were used as the primary metric, with $N = 20$ terms per period for both unigrams and bigrams. Mean normalisation was applied to account for corpus size imbalance. The early group consisted of 1991--1995, 1996--2000, and 2001--2005; the late group consisted of 2011--2015 and 2016--2021.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/Unigram_Keyword_Trends.png}
\caption{Normalised TF-IDF scores for selected 
unigram terms across periods.}
\label{fig:keyword_trends}
\end{figure}

The results confirmed the hypothesis. Among unigrams, the specific terms that emerged in the late periods included \textit{neural}, \textit{learning}, \textit{models}, \textit{training}, and \textit{tasks}, none of which featured in the top-20 lists of any early period. The specific terms that disappeared included \textit{grammar}, \textit{parsing}, \textit{unification}, \textit{syntactic}, and \textit{statistical}. At the bigram level, the phrase-level terms that newly entered the top-ranked lists included \textit{deep learning}, \textit{pre trained}, \textit{language model}, and \textit{word embeddings}, while \textit{finite state}, \textit{tree adjoining}, and \textit{hidden markov} were absent from all late-period lists entirely. These specific vocabulary replacements align with documented developments in NLP history, including the Transformer architecture in 2017 and BERT in 2018 \citep{vaswani_attention_2023, devlin_bert_2019}.

Keyword shift analysis offers high interpretability, as each finding traces directly to named vocabulary items. This contrasts with cosine distance, which quantifies the magnitude of change but cannot identify which specific terms drove it.
...
\subsection{Cosine Distance Analysis}
...
The above table, table~\ref{tab:cosine-distances} reports the cosine distances for all period transitions across all four representations. The largest shift detected by each method is highlighted in bold in Table~\ref{tab:cosine-distances}.
...
Directional analysis reveals a representational divide. TF-IDF unigram (-31.9°), LDA at (-5.7°) and TF-IDF bigram (19.4°) cluster within 51.4°, while SBERT at 42.9° shows only WEAK agreement with unigram (74.8°). TF-IDF and LDA both operate on vocabulary-level structure while SBERT's capture a different dominant variance axis. The breakpoint consensus in Table~\ref{tab:traj-stats} nonetheless stands as the primary, more robust finding.

Axis 2 cosine distance results showed extra temporal details. TF-IDF cosine distance reaches its peak at 2001-2005 to 2006-2010, SBERT peaks at 2006-2010 to 2011-2015, and LDA peaks at 2011-2015 to 2016-2021. It suggests that NLP's paradigm shift spreads upward through representational layers. Vocabulary first, semantic meaning second, thematic structure last. The NLP's semantic and topical space was already reorganising during 2001-2010, even as its lexical space remained stable.

%\paragraph{Interpretability.}
The methods differ in how directly their output can be communicated. Keyword shift analysis and TF-IDF produce named vocabulary lists any reader can verify. Cosine distance quantifies magnitude but not source. LDA requires manual topic-word inspection. SBERT trajectories require probing experiments. Within the same method, we can at least compare PCA centroid positions across panels and use UMAP to qualify structural observation. For non-specialist communication, TF-IDF and keyword shift provide the best balance. SBERT is preferred to catch hidden semantic changes where vocabulary is stable. 

%\paragraph{Alignment with known NLP history.}
The 2011-2015 to 2016-2021 breakpoint matches the appearance of Transformer architecture (2017) and BERT and GPT-style pre-training (2018) \citep{pramanick_diachronic_2023, hall_studying_2008}. The earlier TF-IDF cosine peaks at 2001-2005 to 2006-2010 also map to the displacement of rule-based methods by SVMs and CRFs. \citep{hall_studying_2008}. Taken together, these three perspectives support a single integrated reading. NLP undergo a layered reorganization that began earlier and consolidated at that boundary. 
...
\begin{table}[t]
  ...
  \caption{Method evaluation on three criteria (H\,=\,High,
    M\,=\,Medium, L\,=\,Low, Cons.\,=\,Consistency, Interp\,=\,interpretability, Align\,=\,alignment)}
  \label{tab:eval}
\end{table}
...
NLP research focus has changed most decisively at the 2011-2015 to 2016-2021 boundary, marking the consolidation of the transformer era. The full multi-layered cascade pattern is only visible by combining representation methods and comparison metics.
...
SBERT and LDA extend the TF-IDF findings by showing that NLP's evolution was not only a vocabulary shift, but also a semantic and thematic change. SBERT shows gradual semantic drift across the timeline, while the close distance between 2011-2015 and 2016-2021 suggests continuity between deep learning and transformer research. LDA supports this by showing that earlier structural approaches became less dominant, while later research focused more on neural models and transformer-based methods. These results suggest that vocabulary change reflects broader changes in research meaning and topic structure. One open challenge is interpretability: SBERT distances are hard to explain directly, and LDA topic labels require manual judgement.
...
Cosine distance analysis showed that the NLP shift over the years depends on which representation is used to measure it. Applied among four representations, TF-IDF unigram and bigram both indicated the largest vocabulary shift from period 2001-2005 to 2006-2010, SBERT identified the shift one period later 2006-2010 to 2011-2015, and LDA identified the shift one period later 2011-2015 to 2016-2021. One open challenge remains: the corpus is heavily imbalanced, with 88\% of abstracts concentrated in the 2016--2021 period. This raises the question of whether the detected shifts reflect field-wide transitions or are partly an artefact of the growing volume of abstracts in recent years inflating the stability of later period centroids.

% ── Member 5: forward look (final paragraph) ──────────────────────────────

 The strongest findings came from triangulating across representations and metrics. No single method gave the complete picture. In lexical space, NLP's pre-transformer history looks like a stable plateau ending in a sudden break. But in semantic and topical space, it appears as a continuous non-linear reorganization.
